...
export default function RepoList({ repoDetails, pageNo }: RepoDetailSection) {
  const limit = 4
  return (
    <section className="p-4 grow">
      ...
      {repoDetails?.length > 0 &&
        repoDetails.map((e, index) => {
          if( index >= (pageNo - 1)*limit && index < pageNo * limit)
          return (
            <div className="flex justify-between border-t-2 border-gray-200 items-center" key={index}>
              <div className="mr-1 m-2">
                ...
                <div className="flex gap-2 items-center py-2">
                  ...
                  <p className="font-semibold text-[gray] text-sm">{e?.watchers}</p>
                  <p className="font-semibold text-sm">Watches</p>
                </div>
              </div>
              <Button  label="View" type="button" variant="primary h-fit rounded-lg" link={e?.html_url || ''}/>
            </div>
          );
        })}
        <Pagination totalCount={repoDetails?.length || 0} startIndex={pageNo} limit={limit}/>
    </section>
  );
}
